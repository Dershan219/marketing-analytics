...
Grouping the data to calculate recency, frequency, and monetary value for each customer.
On top of that, I gave them rankings accordingly for sake of quick comparison. 
```{r}
retail.rfm <-
...
Next, we can calculate the RFM scores and give each customer a cluster.
There are infinite ways of doing this, but I chose to give them scores according to their quartile ranks,
and further used kmeans to cluster the customers into 5 groups.
```{r}
...
The BGNBD model can be quite helpful for determining customers' survival rate,
as well as their expected number of transactions in a given time frame. 
```{r}
library(BTYD)
library(BTYDplus)
